...
export class Entity<T extends IComponent> implements IEntity<T> {
  ...
  public owners: Bag<any> = new Bag<any>()
  ...
  removeAllComponents() {
    const _components = this._components
    const componentsLength = _components.length
  ...
  destroy(): void {
    ...
    this.onComponentAdd.clear()
    this.onComponentUpdate.clear()
    this.onComponentRemove.clear()
  ...
  private _getComponentIndex(typeOrIndex: ComponentOrIndex<T>): number {
    return (typeOrIndex as number) ? (typeOrIndex as number) : this._pool.getIndex(typeOrIndex)
  }
