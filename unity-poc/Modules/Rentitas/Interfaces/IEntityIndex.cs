...
//            _index.Remove(key);
    ...
    public static class EntityIndexHashSetCache<TPool>
        where TPool : class, IComponent
    {
        public static int CreatedCount;
        private static Stack<HashSet<Entity<TPool>>> availableSets;

        static EntityIndexHashSetCache()
        {
            CreatedCount = 0;
            availableSets = new Stack<HashSet<Entity<TPool>>>(); 
        }

        public static HashSet<Entity<TPool>> GetSet()
        {
            CreatedCount++;
            
            if (availableSets.Count == 0)
            {
                return new HashSet<Entity<TPool>>(EntityEqualityComparer<TPool>.Comparer);
            }

            return availableSets.Pop();
        }

        public static void ReturnSet(HashSet<Entity<TPool>> set)
        {
            CreatedCount--;
        ...
        where TPool : class, IComponent
    {
        ...
        public HashSet<Entity<TPool>> GetEntities(TKey key)
        {
            ...
            {
                entities = EntityIndexHashSetCache<TPool>.GetSet();
        ...
        {
            var key = _getKey(entity, component);
            var set = GetEntities(_getKey(entity, component));
            if (set.Remove(entity) && set.Count == 0)
            {
                ForgetSet(key, set);
            }
            entity.Release(this);
        ...
        private void ForgetSet(TKey key, HashSet<Entity<TPool>> set)
        {
            _index.Remove(key);
            EntityIndexHashSetCache<TPool>.ReturnSet(set);
        }
    }
